...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if (pgdir[PDX(va)] & PTE_P)
		pgtab = KADDR(PTE_ADDR(pgdir[PDX(va)]));
	else if (create && (r = page_alloc(ALLOC_ZERO)) != 0) {
		r->pp_ref++;
		pgdir[PDX(va)] = page2pa(r) | PTE_P | PTE_W | PTE_U;
		pgtab = KADDR(PTE_ADDR(pgdir[PDX(va)]));
	} else 
		return NULL;
	return pgtab + PTX(va);
...
{
	pte_t *pe;
	if (!(pe = pgdir_walk(pgdir, va, true)))
		return -E_NO_MEM;
	pp->pp_ref++;
	page_remove(pgdir, va);
	*pe = page2pa(pp) | perm | PTE_P;
	return 0;
}
...
{
	pte_t *r;
	if ((r = pgdir_walk(pgdir, va, false)) == NULL)
		return NULL;
	if (pte_store)
		*pte_store = r;
	return pa2page(PTE_ADDR(*r));
}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	// LAB 3: Your code here.
	pte_t *pe;
	len = ROUNDUP(len + (uintptr_t) va % PGSIZE, PGSIZE);
	while (len) {
		// NOTE these code are quite ugly here.. but I'm too lazy to change them
		if (!(pe = pgdir_walk(env->env_pgdir, ROUNDDOWN(va, PGSIZE), false))) {
		...
		// NOTE there are definitely other security problems
